...
function VirtualList() {
  ...
  const scroll = useRef<HTMLDivElement>(null); // 获取scroll滚动元素
  const box = useRef<HTMLDivElement>(null); //获取元素容器高度
  ...
  useEffect(() => {
    ...
    setDataList(dataList as any)
    setPosition([0,renderCount])
  }, [])
  const handleScroll = () => {
    const { scrollTop } = scroll.current as HTMLElement
